...
@Component
public class GroupCampaignGameNotification implements ManageGameNotification {
    private static Logger logger = LoggerFactory.getLogger(GroupCampaignGameNotification.class);
    ...
    @Autowired
    GroupCampaignGameStatusManager gameStatusManager;

    @Autowired
    GroupCampaignChallengeNotification challengeNotification;

	@EventListener(ContextRefreshedEvent.class)
	public void init() {
		gamificationMessageQueueManager.setManageGameNotification(this, Type.group);
		List<Campaign> list = campaignRepository.findByType(Type.group, Sort.by(Sort.Direction.DESC, "dateFrom"));
		...
		if(!gameIds.isEmpty()) {
		    ...
		    logger.info(String.format("campaigns type %s subscribes to games", Type.group));
		}
	}
